...
import { Loader2, Mail, Lock, Wind, Users, Shield, Headphones, Monitor } from "lucide-react";
...
export default function Auth() {
  ...
  const [selectedRole, setSelectedRole] = useState<string>('sistema');
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (teamCode && signUpData.user) {
        ...
        const roleLabel = ROLE_INFO[selectedRole]?.label || selectedRole;
        toast({
          title: "🎉 Bem-vindo à equipe!",
          description: `Conta criada com acesso: ${roleLabel}`
        });
      } else {
        ...
        {showForgotPassword ? (
          ...
          <Card className="backdrop-blur-xl bg-[#0d1f3c]/80 border border-cyan-500/20 rounded-2xl shadow-[0_0_50px_rgba(6,182,212,0.15)] animate-scale-in">
            <CardContent className="p-0">
              <Tabs defaultValue={teamCode ? "signup" : "login"} className="w-full">
                ...
                <TabsContent value="signup" className="p-6 space-y-4">
                  ...
                  {teamCode && (
                    <div className="space-y-3 p-4 bg-cyan-500/5 rounded-xl border border-cyan-500/20">
                      <p className="text-xs text-cyan-300 font-semibold uppercase text-center">Escolha sua função</p>
                            ...
                            <button key={key} type="button" onClick={() => setSelectedRole(key)}
                              className={`flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${
                                isSelected
                                  ...
                                  : 'border-cyan-500/10 bg-[#0a1628]/50 hover:border-cyan-500/30'
                              }`}>
                              <div className={`p-2 rounded-lg ${isSelected ? 'bg-cyan-500/20' : 'bg-[#0a1628]'}`}>
                                <Icon className={`w-5 h-5 ${info.color}`} />
                              </div>
                              <div className="flex-1">
                                <p className={`text-sm font-medium ${isSelected ? 'text-white' : 'text-cyan-100/70'}`}>
                                  {info.label}
                                </p>
                                <p className="text-xs text-cyan-200/40">{info.desc}</p>
                              </div>
                              <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                                isSelected ? 'border-cyan-400 bg-cyan-400' : 'border-cyan-500/30'
                              }`}>
