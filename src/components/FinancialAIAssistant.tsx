...
interface Props {
  ...
  buildSnapshot: () => Promise<FinancialSnapshot> | FinancialSnapshot;
  onAction?: (actionId: string) => void;
}

type Msg = { role: "user" | "assistant"; content: string };

export default function FinancialAIAssistant({ open, onOpenChange, buildSnapshot, onAction }: Props) {
  ...
  // Carrega snapshot toda vez que abre
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    (async () => {
  ...
  const callAI = async (userMsg: string, history: Msg[]) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("financial-ai-assistant", {
        body: { message: userMsg, history, snapshot },
      });
  ...
  const runAutoDiagnose = async () => {
    if (!snapshot) return;
    const next = [...msgs, { role: "user" as const, content: "Diagnostique meu mês e me mostre o que corrigir." }];
    setMsgs(next);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        ...
        {/* Painel de diagnóstico automático */}
        {snapshot && (
            ...
            >
              <span className="flex items-center gap-2">
                {snapshot.issues.length === 0 ? (
                  <><CheckCircle2 className="h-4 w-4 text-emerald-600" /> Tudo certo no mês {snapshot.month}</>
                ) : (
            ...
            {diagOpen && (
              <div className="px-3 pb-3 space-y-2">
                <div className="grid grid-cols-2 gap-1 text-[11px]">
                  <span>Entradas: <b>R$ {snapshot.cards.totalEntradas.toFixed(2)}</b></span>
                  <span>Despesas: <b>R$ {snapshot.cards.totalDespesas.toFixed(2)}</b></span>
                  <span>Saldo: <b className={snapshot.cards.saldo < 0 ? "text-red-600" : "text-emerald-600"}>R$ {snapshot.cards.saldo.toFixed(2)}</b></span>
                  <span>Contratos: <b>R$ {snapshot.cards.contratos.toFixed(2)}</b></span>
                </div>
                {snapshot.issues.length > 0 && (
        ...
        <ScrollArea className="flex-1 min-h-[200px] border rounded-md p-2" ref={scrollRef as any}>
          {msgs.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-6">
              Pergunte: "Por que o card Contratos está R$ X?", "Onde foi o R$ Y?", "Como corrijo o saldo?"...
            </p>
          ) : (
        ...
        <div className="flex gap-2">
          <Input
            ...
            placeholder="Pergunte sobre o financeiro do mês..."
            disabled={loading}
          />
