...
import { Copy, ExternalLink, Link2, LogIn, Globe, Share2, QrCode } from "lucide-react";
...
export const AdminShareTab: React.FC = () => {
  ...
  const shareLink = async (url: string, title: string) => {
    if (navigator.share) {
      try {
        await navigator.share({ title, url, text: `Confira o ${title}!` });
      } catch {}
    } else {
  ...
  const links = [
    { 
      icon: Globe, label: 'Landing Page', desc: 'Página de vendas principal', 
      url: landingUrl, color: 'text-cyan-400', bg: 'from-cyan-500/20 to-blue-500/20' 
    },
    { 
      icon: LogIn, label: 'Página de Login', desc: 'Link direto para login', 
      url: loginUrl, color: 'text-green-400', bg: 'from-green-500/20 to-emerald-500/20' 
    },
    { 
      icon: Link2, label: 'Página de Cadastro', desc: 'Link direto para criar conta', 
      url: cadastroUrl, color: 'text-purple-400', bg: 'from-purple-500/20 to-pink-500/20' 
    },
  ];
    ...
    <div className="space-y-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <Input value={link.url} readOnly 
                      className="bg-[#0f0f17] border-[#2a2a3a] text-cyan-300 text-xs h-8 font-mono" />
                  </div>
                </div>
      ...
      <Card className="bg-[#1a1a24] border-[#2a2a3a]">
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-gray-400 text-xs">Clique para copiar e enviar para clientes/parceiros</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {[
            ...
            { label: 'Indicação com benefício', msg: `❄️ Estou usando um sistema incrível pra gerenciar meus serviços de ar condicionado. Dá uma olhada: ${landingUrl}` },
            { label: 'Link de cadastro direto', msg: `Cria sua conta aqui pra começar a usar o AC Service Pro: ${cadastroUrl}` },
          ].map((tmpl, i) => (
